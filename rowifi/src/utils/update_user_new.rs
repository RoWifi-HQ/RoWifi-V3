...
use rowifi_models_new::{user::RoGuildUser, guild::{RoGuild, BlacklistActionType}, bind::Bind, discord::id::RoleId, rolang::RoCommandUser, roblox::user::PartialUser as RobloxUser};
use rowifi_models::roblox::id::{UserId as RobloxUserId, AssetId as RobloxAssetId};
...
impl UpdateUser<'_> {
    pub async fn execute(self) -> UpdateUserResult {
        ...
        let roblox_user = match self.ctx.roblox.get_user(user_id, self.bypass_roblox_cache).await {
            Ok(r) => RobloxUser {
                id: rowifi_models_new::roblox::id::UserId(r.id.0),
                name: r.name,
