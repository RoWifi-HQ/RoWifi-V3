...
#[derive(FromArgs)]
pub struct AdminAddArguments {
    #[arg(rest, help = "List of all roles to be added as `RoWifi Admin`")]
    pub roles: String,
}

pub async fn admin_add(ctx: CommandContext, args: AdminAddArguments) -> CommandResult {
    ...
    let update = doc! {"$push": {"Settings.AdminRoles": {"$each": roles_to_add.clone()}}};
