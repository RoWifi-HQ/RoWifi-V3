...
use rowifi_models::{
    guild::{GuildType, RoGuild, backup::GuildBackup}, user::{RoUser, UserFlags},
    bind::*, rolang::RoCommand
};
...
pub async fn backup_restore(ctx: CommandContext, args: BackupArguments) -> CommandResult {
    match ctx.bot.database.query_opt::<RoUser>("SELECT * FROM users WHERE discord_id = $1", &[&(ctx.author.id.get() as i64)]).await? {
    ...
    let backup = match ctx.bot.database.query_opt::<GuildBackup>("SELECT * FROM backup WHERE discord_id = $1 AND name = $2", &[&(ctx.author.id.get() as i64), &name]).await? {
        Some(b) => b,
        None => {
            let embed = EmbedBuilder::new()
                .default_data()
    ...
    let all_roles = backup.data.0.binds.iter().map(|b| b.discord_roles()).flatten().map(|s| s.clone()).unique().collect::<Vec<_>>();
    ...
    for r in all_roles {
        if let Some(existing) = existing_roles.get(&r) {
            ...
            let role = ctx.bot.http.create_role(guild_id).name(&r).exec().await?.model().await?;
    ...
    let verification_roles = data.verification_roles.iter().map(|v| roles_map.get(v).map(|r| r.get() as i64)).flatten().collect();
    let verified_roles = data.verified_roles.iter().map(|v| roles_map.get(v).map(|r| r.get() as i64)).flatten().collect();

    let binds = data.binds.into_iter().map(|b| {
        let discord_roles = b.discord_roles().iter().map(|s| roles_map.get(s).map(|r| r.get() as i64)).flatten().collect();
    ...
    let delete_guild = transaction.prepare_cached("DELETE FROM guilds WHERE guild_id = $1").await?;
    transaction.execute(&delete_guild, &[&guild.guild_id]).await?;
    ...
    transaction.execute(&insert_guild, &[&guild.guild_id]).await?;

    let delete_binds = transaction.prepare_cached("DELETE FROM binds WHERE guild_id = $1").await?;
    transaction.execute(&delete_binds, &[&guild.guild_id]).await?;
    ...
    for bind in binds {
        match bind {
            Bind::Rank(r) => transaction.execute(&add_rank, &[&BindType::Rank, &guild.guild_id, &r.group_id, &r.group_rank_id, &r.roblox_rank_id, &r.template, &r.priority, &r.discord_roles]).await?,
            Bind::Group(g) => transaction.execute(&add_group, &[&BindType::Group, &guild.guild_id, &g.group_id, &g.discord_roles, &g.priority, &g.template]).await?,
            Bind::Custom(c) => transaction.execute(&add_custom, &[&BindType::Custom, &guild.guild_id, &c.custom_bind_id, &c.discord_roles, &c.code, &c.priority, &c.template]).await?,
            Bind::Asset(a) => transaction.execute(&add_asset, &[&BindType::Asset, &guild.guild_id, &a.asset_id, &a.asset_type, &a.discord_roles, &a.priority, &a.template]).await?
        };
    }
