...
use rowifi_models::{user::{RoUser, UserFlags}, guild::backup::{GuildBackup, GuildBackupData}, discord::id::RoleId, bind::*};
...
pub async fn backup_new(ctx: CommandContext, args: BackupArguments) -> CommandResult {
    ...
    let verification_roles = guild.verification_roles.iter().map(|r| {
            roles.get(&RoleId::new(*r as u64).unwrap()).cloned()
        })
        .flatten()
        .collect();
    let verified_roles = guild.verified_roles.iter().map(|r| {
            roles.get(&RoleId::new(*r as u64).unwrap()).cloned()
        })
        .flatten()
        .collect();
    let binds = binds.into_iter().map(|b| {
            let discord_roles = b.discord_roles().iter().map(|r| {
                roles.get(&RoleId::new(*r as u64).unwrap()).cloned()
            })
            .flatten()
            .collect();
