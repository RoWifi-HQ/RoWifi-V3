...
pub async fn blacklists_delete(ctx: &Context, msg: &Message, mut args: Arguments<'fut>) -> CommandResult {
    ...
    if blacklist.is_none() {
        let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
            .title("Blacklist Deletion Failed").unwrap()
            .description("A blacklist with the given id was not found").unwrap()
            .build().unwrap();
    ...
    let e = EmbedBuilder::new().default_data().color(Color::DarkGreen as u32).unwrap()
        .title("Blacklist Deletion Successful").unwrap()
        .description("The given blacklist was successfully deleted").unwrap()
        .build().unwrap();
    ...
    let log_embed = EmbedBuilder::new().default_data().title(format!("Action by {}", msg.author.name)).unwrap()
        .description("Blacklist Deletion").unwrap()
