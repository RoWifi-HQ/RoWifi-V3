...
pub async fn groupbinds_modify(ctx: &Context, msg: &Message, mut args: Arguments<'fut>) -> CommandResult {
   ...
   if guild.groupbinds.iter().any(|c| c.group_id == group_id) {
        let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
            .title("Group Bind Modification Failed").unwrap()
            .description(format!("There was no bind found with id {}", group_id)).unwrap()
            .build().unwrap();
    ...
    let desc = if field.eq_ignore_ascii_case("roles-add") {
        ...
        return Err(CommandError::ParseArgument(field.into(), "Field".into(), "`roles-add`, `roles-remove`".into()).into())
    };
    ...
    let log_embed = EmbedBuilder::new().default_data()
        ...
        .description("Group Bind Modification").unwrap()
...
async fn add_roles(ctx: &Context, guild: &RoGuild, group_id: i64, mut args: Arguments<'_>) -> Result<Vec<u64>, RoError> {
...
async fn remove_roles(ctx: &Context, guild: &RoGuild, group_id: i64, mut args: Arguments<'_>) -> Result<Vec<u64>, RoError> {
    let mut role_ids = Vec::new();
    while let Some(r) = args.next() {
        if let Some(r) = parse_role(r) {
            role_ids.push(r);
