...
#[command]
pub async fn premium_transfer(ctx: &Context, msg: &Message, mut args: Arguments<'fut>) -> CommandResult {
    ...
    if let Some(premium_user) = premium_user {
        if premium_user.premium_owner.is_some() {
            let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
                .title("Premium Transfer Failed").unwrap()
                .description("You may not transfer a premium that you do not own").unwrap()
                .build().unwrap();
        ...
        let to_transfer_id = match args.next().map(|a| a.parse::<i64>()) {
            ...
            _ => {
                let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
                    .title("Premium Transfer Failed").unwrap()
                    .description("You must specify a user id to transfer to").unwrap()
                    .build().unwrap();
                let _ = ctx.http.create_message(msg.channel_id).embed(embed).unwrap().await?;
                return Ok(())
            }
        };
        if ctx.database.get_premium(to_transfer_id as u64).await?.is_some() {
            let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
                .title("Premium Transfer Failed").unwrap()
                .description("You cannot transfer premium to a user who already has premium").unwrap()
                .build().unwrap();
            let _ = ctx.http.create_message(msg.channel_id).embed(embed).unwrap().await?;
            return Ok(())
        }
        ...
        let embed = EmbedBuilder::new().default_data().color(Color::DarkGreen as u32).unwrap()
            .title("Premium Transfer Successful").unwrap()
            .build().unwrap();
        let _ = ctx.http.create_message(msg.channel_id).embed(embed).unwrap().await?;
    } else if let Some(transferred_premium_user) = ctx.database.get_transferred_premium(msg.author.id.0).await? {
        ctx.database.delete_premium(transferred_premium_user.discord_id as u64).await?;
        ...
        let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
            .title("Premium Transfer Failed").unwrap()
            .description("You do not have a premium subscription").unwrap()
            .build().unwrap();
        let _ = ctx.http.create_message(msg.channel_id).embed(embed).unwrap().await?;
    }
    Ok(())
