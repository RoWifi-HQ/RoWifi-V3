use crate::framework::{prelude::*, structures::HelpCommand, CommandMap, parser::ParseError};
...
#[command]
pub async fn help(ctx: &Context, msg: &Message, args: Arguments<'fut>, commands: &[(&'static Command, CommandMap)]) -> CommandResult {
...
async fn global_help(ctx: &Context, msg: &Message, commands: &[(&'static Command, CommandMap)]) -> Result<(), RoError> {
    let mut embed = EmbedBuilder::new().default_data()
        .title("Help").unwrap()
        .description("Listing all top-level commands").unwrap();
...
async fn specific_help(ctx: &Context, msg: &Message, args: Arguments<'_>, commands: &[(&'static Command, CommandMap)]) -> Result<(), RoError> {
    let mut embed = EmbedBuilder::new().default_data()
        .title("Help").unwrap();
    ...
    if let Ok(command) = last {
        embed = embed.description(format!("`{}`: {}", command.options.names[0], command.options.desc.unwrap_or("None"))).unwrap();
        if command.options.names.len() > 1 {
            let aliases = command.options.names[1..].iter().map(|a| format!("`{}`", a)).join(" ");
            embed = embed.field(EmbedFieldBuilder::new("Aliases",  aliases).unwrap());
        }
        let mut examples = command.options.examples.iter().map(|e| format!("`{}`", e)).join("\n");
        if examples.is_empty() {examples = "None".into();}

        embed = embed.field(EmbedFieldBuilder::new("Usage", command.options.usage.unwrap_or("None")).unwrap())
            .field(EmbedFieldBuilder::new("Examples", examples).unwrap());
...
fn parse_command(mut args: Arguments, map: &CommandMap) -> Result<&'static Command, ParseError> {
    ...
    Err(ParseError::UnrecognisedCommand(Some("Arguments ended".into())))
}