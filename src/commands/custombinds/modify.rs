...
pub async fn custombinds_modify(ctx: &Context, msg: &Message, mut args: Arguments<'fut>) -> CommandResult {
    ...
    let bind_id =match args.next() {
        Some(a) => match a.parse::<i64>() {
            Ok(a) => a,
            Err(_) => return Err(CommandError::ParseArgument(a.into(), "Bind ID".into(), "Number".into()).into())
        },
        None => return Ok(())
    };
    ...
    let bind = match guild.custombinds.iter().find(|c| c.id == bind_id) {
        ...
        None => {
            let embed = EmbedBuilder::new().default_data().color(Color::Red as u32).unwrap()
                ...
                .description(format!("There was no bind found with id {}", bind_id)).unwrap()
                .build().unwrap();
    ...
    let log_embed = EmbedBuilder::new().default_data()
        ...
        .description("Custom Bind Modification").unwrap()
...
async fn modify_code(ctx: &Context, msg: &Message, guild: &RoGuild, bind_id: i64, mut args: Arguments<'_>) -> Result<Option<String>, RoError> {
    let code = args.join(" ");
...
async fn modify_prefix(ctx: &Context, guild: &RoGuild, bind_id: i64, prefix: Option<&str>) -> Result<String, RoError> {
...
async fn modify_priority(ctx: &Context, guild: &RoGuild, bind_id: i64, priority: Option<&str>) -> Result<i64, RoError> {
