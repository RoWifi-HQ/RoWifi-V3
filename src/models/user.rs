...
impl RoUser {
    pub async fn update(
            ...
            if let Some(success) = success {
                match guild.settings.blacklist_action {
                    ...
                    BlacklistActionType::Kick => {
                        let _ = http.remove_guild_member(server.id, member.user.id).await;
                    }
                    BlacklistActionType::Ban => {
                        let _ = http.create_ban(server.id, member.user.id).await;
                    }
                };
        ...
        for asset in &guild.assetbinds {
            if rbx
                .has_asset(self.roblox_id, asset.id, &asset.asset_type.to_string())
                .await?
            {
