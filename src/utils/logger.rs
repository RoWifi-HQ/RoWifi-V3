...
impl Logger {
    ...
    pub async fn log_debug(&self, ctx: &Context, text: &str) {
        let _ = ctx.http.execute_webhook_from_url(&self.debug_webhook).unwrap().content(text.to_string()).await;
    }

    pub async fn log_event(&self, ctx: &Context, embed: Embed) {
        let _ = ctx.http.execute_webhook_from_url(&self.main_webhook).unwrap().embeds(vec![embed]).await;
    }

    pub async fn log_premium(&self, ctx: &Context, text: &str) {
        let _ = ctx.http.execute_webhook_from_url(&self.premium_webhook).unwrap().content(text.to_string()).await;
    }
}