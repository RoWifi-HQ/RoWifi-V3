...
impl Patreon {
    ...
    pub async fn get_patron(&self, discord_id: u64) -> Result<(Option<String>, Option<String>), RoError> {
                ...
                if user["type"].as_str().unwrap().eq("user") {
                    ...
                    if let Some(Ok(disc)) = disc.as_str().map(|s| s.parse::<u64>()) {
                        if disc == discord_id {
                            ...
                            for u in users {
                                if u["relationships"]["user"]["data"]["id"].as_str().unwrap() == patreon_id {
                                    let tiers = u["relationships"]["currently_entitled_tiers"]["data"].as_array().unwrap();
                                    if !tiers.is_empty() {
                                        return Ok((Some(patreon_id.to_string()), tiers[0]["id"].as_str().map(|s| s.to_owned())))
                                    }
